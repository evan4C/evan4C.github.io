...
2. `sed`：流编辑器，用于替换、删除、插入文本，支持基本正则
3. `awk`：文本分析工具，支持正则匹配，用于数据筛选、统计等
4. `find`：文件搜索
5. `tar`/`zip`：压缩解压
...
Tmux（Terminal Multiplexer）允许用户在一个窗口内创建多个终端会话。用户可以从命令行分离和重新连接到会话，将终端分割成多个窗格，并将它们组织成多个窗口。主要优势
- **会话持久性**：即使断开 SSH 连接或关闭终端，也能保持会话运行
- **多窗格和多窗口**：通过分割视图和多个窗口来组织工作
- **结对编程**：与其他用户共享终端会话
- **可定制性**：可根据工作流程进行广泛配置
- **可脚本化**：自动化复杂的终端设置
